...
export async function GET() {
  ...
  const rows = await sql`
    SELECT id, username, role, created_by, created_at, is_active, permissions, role_id, discord_id
    FROM admin_accounts
...
export async function POST(req: NextRequest) {
  ...
  try {
    const rows = await sql`
      ...
      RETURNING id, username, role, created_by, created_at, is_active, permissions, role_id, discord_id
    `
...
export async function PATCH(req: NextRequest) {
  if (!(await isFounder())) return Response.json({ error: "Yetkisiz" }, { status: 403 })
  const { id, is_active, role, password, permissions, role_id, discord_id } = await req.json()
  ...
  if (discord_id !== undefined) {
    if (discord_id === null || discord_id === "") {
      await sql`UPDATE admin_accounts SET discord_id = NULL WHERE id = ${id}`
    } else {
      await sql`UPDATE admin_accounts SET discord_id = ${discord_id} WHERE id = ${id}`
    }
  }
  const rows = await sql`
    SELECT id, username, role, created_by, created_at, is_active, permissions, role_id, discord_id
